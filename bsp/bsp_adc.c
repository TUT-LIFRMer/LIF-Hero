...
volatile my_fp32 voltage_vrefint_proportion = 8.0586080586080586080586080586081e-4f;
...
my_fp32 get_temprate(void)
{
    uint16_t adcx = 0;
    my_fp32 temperate;

    adcx = adcx_get_chx_value(&hadc1, ADC_CHANNEL_TEMPSENSOR);
    temperate = (my_fp32)adcx * voltage_vrefint_proportion;
...
my_fp32 get_battery_voltage(void)
{
    my_fp32 voltage;
    uint16_t adcx = 0;

    adcx = adcx_get_chx_value(&hadc3, ADC_CHANNEL_8);
    voltage =  (my_fp32)adcx * voltage_vrefint_proportion * 10.090909090909090909090909090909f;
